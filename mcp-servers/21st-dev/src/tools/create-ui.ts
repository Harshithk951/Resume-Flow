...
export class CreateUiTool extends BaseTool {
  ...
  schema = z.object({
    ...
    searchQuery: z
      .string()
      .describe(
        "Generate a search query for 21st.dev to find a UI component that matches the user's message. Must be two-four words max or phrase."
      ),
    ...
    standaloneRequestQuery: z
      .string()
      .describe(
        "Formulate what component the user wants to create based on their message, possible chat history, and context."
      ),
  });
  ...
  async execute({
    ...
    try {
      ...
      const prompt =
        data || "// No component data received. Please try again.";
