import { z } from "zod";
import { BaseTool } from "../utils/base-tool.js";
import { twentyFirstClient } from "../utils/http-client.js";

const FETCH_UI_TOOL_NAME = "21st_magic_component_inspiration";
const FETCH_UI_TOOL_DESCRIPTION = `
"Use this tool when the user wants to see components, get inspiration, or fetch data and previews from 21st.dev.
This tool returns JSON data of matching components without generating new code.
After calling this tool, you must edit or add files to integrate the snippet into the codebase."
`;

interface FetchUiResponse {
  text: string;
}

export class FetchUiTool extends BaseTool {
  name = FETCH_UI_TOOL_NAME;
  description = FETCH_UI_TOOL_DESCRIPTION;

  schema = z.object({
    message: z.string().describe("Full user's message"),
    searchQuery: z
      .string()
      .describe(
        "Search query for 21st.dev to find UI components matching the user's message. Must be two-four words max or phrase."
      ),
  });

  async execute({ message, searchQuery }: z.infer<typeof this.schema>) {
    try {
      const { data } = await twentyFirstClient.post<FetchUiResponse>(
        "/api/fetch-ui",
        {
          message,
          searchQuery,
        }
      );

      return {
        content: [
          {
            type: "text" as const,
            text: data.text,
          },
        ],
      };
    } catch (error) {
      console.error("Error executing tool", error);
      throw error;
    }
  }
}
